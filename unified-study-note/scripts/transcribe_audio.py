...
def resolve_omniroute_credentials():
    ...
    for k, v in os.environ.items():
        if k.startswith("HERMES_") and k.endswith("_API_KEY"):
            parts = k[len("HERMES_"):-len("_API_KEY")].split("_")
            if len(parts) >= 5:
                ip = ".".join(parts[:4])
                port = parts[4]
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe audio recordings using OmniRoute STT model 'best-stt' with chunking. The audio contains a lecture or practical class from a course in a MSc Data Science program, so try to catch and trascribe correctly the technical terms.")
